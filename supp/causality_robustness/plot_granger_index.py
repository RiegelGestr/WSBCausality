...
for window in windows:
    ...
    with open("data/granger_index_GME_with_twitter_"+str(window)+".csv") as inpuf:
    ...
    ax.legend(fontsize = fontsize*0.5,bbox_to_anchor=[0.5, +1.07],loc='upper center', ncol = 6, frameon=False)
    ################################################################################################
    props = dict(boxstyle='round', facecolor='grey', alpha = 0.3)
    ax.text(0.85, 0.95, "Window = {window}".format(window = window),
    ...
    fig.savefig("fig/fig_2_A_supp_{window}.pdf".format(window = window))