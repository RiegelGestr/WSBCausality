...
for stock in stocks:
    xA = df["GME"+"_users"]
    ts = df["GME_"+stock+"_overlap"]
    ax.plot(xs,ts, lw = 30,label = "GME-"+stock,color = dict_color["GME_"+stock+"_overlap"])
...
ax.axhline(0.5,xmin = 0.0,xmax = 1.0, color = "#9AA3B3", lw = 16,ls = "solid")
ax.set_ylabel("Overlap",fontsize = fontsize*0.7,rotation = 90,labelpad = 48)
ax.set_ylim([0,1.0])
